...
void list::createEmpty(List& l){
    l.size = 0;
    l.maxsize = BLOCKDIM;
    Elem* aux = new Elem[BLOCKDIM]; // BLOCKDIM = 1
    l.list = aux;
}
...
void list::add(int pos, Elem e, List& l) // aggiunge e nella posizione pos
{
   ...
   if (l.size == l.maxsize) // l'array e' pieno, per poter inserire un nuovo elemento devo ridimensionare l'array espandendolo; l'unico modo e' creare un array piu' grande e copiare in esso gli elementi di l
      ...
      List auxList; // creo una nuova lista
      auxList.size = l.size+1; // di size rispetto a l aumentata di uno
      auxList.maxsize = l.maxsize + BLOCKDIM; // aumenta la dimensione massima dell array di uno
      Elem* auxArray = new Elem[auxList.maxsize]; // crea un nuovo array di della dimensione aggiornata
      auxList.list = auxArray;
      for (int i=0; i<pos; ++i)
          auxList.list[i] = l.list[i]; // copio l elemento della lista l in auxlist nella pos i
      auxList.list[pos] = e;   // fino a che non incontro pos e allora ci assegno 'e'
      for (int i=pos+1; i<auxList.size; ++i)
          auxList.list[i] = l.list[i-1]; // continuo a copiare gli elementi dopo 'e' fino a che non termina l
      delete[] l.list; // dealloco l
      l = auxList; // in fine copio la lista cosi fatta in l
      return;
      }
